...
Voxa is a unified 3D scan studio (viewer + cuboid labeler) for industrial LiDAR / point clouds. It replaces the older `3d-labeler` and `industrial-point-labeler` tools. The UI is organized around three modes: **Inspect** (fast review), **Label** (cuboid annotation, optional auto-fit, plus two sub-modes: Fast-labeling, which steps through preseg segments largest-first for one-keypress classification — see `frontend/src/fast-label.jsx` — and Draw, which labels cylindrical objects (pipes/tanks) by drawing centerline paths: the backend extracts all full-res points within a per-path tube radius and paths persist per-session in `sessions/<id>/centerlines.json` — see `frontend/src/draw-mode.jsx` + `backend/labeling/centerline.py`. Draw renders the raw RGB cloud (preseg hues/hulls suppressed, point-size slider in the panel), allows walk nav, and supports point anchors — click a control point, Ctrl+click extends from it (head/tail) or branches off a junction; Ctrl+click on a tube/point multi-selects like presegments, M merges; applied paths auto-hide behind a ◌/● toggle and surface as pointset rows in the right Instances panel), and **Compare** (two finished labelings — session outputs or presegs — diffed per-point: agreement coloring, per-class IoU/precision/recall, confusion matrix).
