...
- **SAM** — label arbitrary/irregular objects with SAM (Segment Anything v3): Shift-drag a 2D box (or type a text prompt for "segment all ‹pipe›"), a **separate GPU sidecar** renders the raw cloud from voxa's exact camera pose, runs SAM, and back-projects the mask(s) to scan-resolution point indices; the returned masks are shown in a review panel (pick which to keep) → each becomes an unconfirmed pointset. The sidecar (`sam_sidecar/`, its own anaconda/CUDA process — NOT voxa's `.venv`) holds the raw 188M-class cloud + SAM model; voxa's viewer is unchanged, used only as a pose+box source. The frontend renders nothing for SAM — the image SAM sees is server-side (`sam_sidecar/render.py`, a numpy perspective splat), which is why the tool needs the raw source and is gated on `raw_source_available`. Frontend: `frontend/src/sam-mode.jsx` (box-drag capture, mask-review, project→apply). Backend proxy: `backend/routes/sam.py` (`POST /api/sam/capture` → sidecar; `POST /api/sam/project` → sidecar indices → the same `apply_reassign(protect_instances=…)` path). The sidecar is reached via `VOXA_SAM_SIDECAR_URL`; scan identity is guarded by a source-fingerprint handshake (409 `{diverged:"source"}` on mismatch) so `/project` can never write indices onto the wrong cloud. Single-view v1 (no multi-view merge yet). See `sam_sidecar/README.md` and `docs/superpowers/specs/2026-07-12-sam-labeling-tool-design.md`.
