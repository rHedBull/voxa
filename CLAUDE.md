...
- **SAM** — label arbitrary/irregular objects with SAM (Segment Anything v3): Shift-drag a 2D box (or type a text prompt for "segment all ‹pipe›"), a **separate GPU sidecar** renders the raw cloud from voxa's exact camera pose, runs SAM, and back-projects the mask(s) to scan-resolution point indices; the returned masks are shown in a large review modal (`frontend/src/sam-mode.jsx::SamReviewModal`) → each kept mask becomes an unconfirmed pointset. The modal shows the full-size overlay image beside (not below) a mask list, each row swatched in the same palette baked into the overlay (`sam-util.js::maskColor`, mirrors `sam_sidecar/main.py::_palette`); a mask can also be picked by clicking it directly in the image — the sidecar additionally returns `mask_index_png_b64`, a lossless per-pixel mask-index map the frontend decodes and hit-tests the click against (`sam-util.js::containPixel` accounts for `object-fit: contain` letterboxing). Click selects only that mask; Ctrl/Cmd/Shift+click adds/removes it from the selection. Projection is single-view v1 — `/project` only keeps points visible (non-occluded) from the exact capture camera, so a mask that fully covers a chunky/curved object in 2D can still back-project to only its near-facing surface in 3D; no multi-view merge yet. The sidecar (`sam_sidecar/`, its own anaconda/CUDA process — NOT voxa's `.venv`) holds the raw 188M-class cloud + SAM model; voxa's viewer is unchanged, used only as a pose+box source. The frontend renders nothing for SAM — the image SAM sees is server-side (`sam_sidecar/render.py`, a numpy perspective splat), which is why the tool needs the raw source and is gated on `raw_source_available`. Backend proxy: `backend/routes/sam.py` (`POST /api/sam/capture` → sidecar; `POST /api/sam/project` → sidecar indices → the same `apply_reassign(protect_instances=…)` path). The sidecar is reached via `VOXA_SAM_SIDECAR_URL`; scan identity is guarded by a source-fingerprint handshake (409 `{diverged:"source"}` on mismatch) so `/project` can never write indices onto the wrong cloud. See `sam_sidecar/README.md` and `docs/superpowers/specs/2026-07-12-sam-labeling-tool-design.md`.

Every apply produces a `kind:'pointset'` instance (the geometric `cuboid` type is retired for new labels — legacy cuboids render display-only, no gizmo). Apply = Ctrl+Enter → class picker, or a class hotkey directly. A per-tool **auto-confirm on apply** toggle is the only deviation from unconfirmed-first (rapid preseg defaults on). Geometric tools (Box, Draw, Beam) reach the backend through **one generic endpoint** `POST /api/segment/apply-shape` (`backend/routes/segment.py` + `backend/labeling/shapes.py`): it resolves a shape (`obb` | `tube`; Beam sends per-beam `obb`s, so no `beam` shape type exists) to the FULL-RES points inside it via `shape_indices`, then `apply_reassign`s them — this is why Box/Draw need the backend (the rendered cloud is subsampled but labels are full-res). `centerline-apply` is a thin `tube`-shaped alias of `apply-shape`.
