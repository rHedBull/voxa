...
- **`backend/labeling/materialize.py`** is the density-agnostic label materializer that consumes the above: `materialize(ctx, resolution)` produces per-point `(positions, colors, class_ids, instance_ids)` at any density — regime A (`≤ scan.ply`: exact index-transfer) or regime B (denser/raw: NN + volumetric max-`seq` replay, box/tube boundaries exact, preseg NN-approximate). The raw cloud resolves via `derivation→sources.json` (`raw_source_available` on `LoadResponse`). It's a pure library (no endpoint yet); the export wizard (Phase B of `docs/superpowers/specs/2026-07-10-export-wizard-design.md`) is its consumer and is not yet built. For a real raw (~156M) cloud, stream `materialize_raw(...)`'s generator — do NOT call `materialize(kind="raw")` (it assembles in memory).
